	...
	<head>
		...
		<script type="text/javascript">
			$(document).ready(function(){
				...
				$("#artista_insert_update_form").validate({
					rules: {
						nombre_artista: "required",
						genero_artista: "required",
						imagen_artista: "required"
					},
					messages: {
						nombre_artista: 'Nombre requerido',
						genero_artista: 'Género requerido',
						imagen_artista: 'Imagen requerida'
					}
				});
		...
		<div id="page">
			...
			<div id="main_view" class="shadow">		
			   ...
			   <div id="contenedor_contenido">		
				...
				<?php if(isset($valor_campos) == false || $valor_campos == false){ ?>
						<p>El artista indicado no existe. </p>
				<?php }else{ 	?>			   	
					<div class="center">
			   			<h1> musicst </h1>
			   			<?php if(isset($insert) == false || $insert == true){ ?>
			   		   			<h3> Agregar artista <?php echo $valor_campos['nombre_artista'] ?></h3>
			   			<?php }else{ ?>
			   					<h3> Actualizar artista <?php echo $valor_campos['nombre_artista'] ?></h3>
			   			<?php } ?>
						...
						<?php } ?>
		   		        ...
		   		        <?php
				          $attributes = array('id' => 'artista_insert_update_form');
				          echo form_open_multipart('admin_edit/insertBDArtista', $attributes); 
				        ?>
						...
						<p class="campo_form_registro">
							<!--Nombre:--> 
							<label for="nombre_artista">Nombre:</label>
							<br/>
							<input type="text" name="nombre_artista" id = "nombre_artista" 
							value="<?php echo $valor_campos['nombre_artista'] ?>"/>
							<br/>
							
						</p>
						<p class="campo_form_registro">
							<!--Nombre:--> 
							<label for="genero_artista">Género:</label>
							<br/>
							<input type="text" name="genero_artista" id = "genero_artista" 
							value="<?php echo $valor_campos['genero_artista'] ?>"/>
							<br/>
