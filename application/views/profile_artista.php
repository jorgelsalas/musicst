		...
		<div id="page">
			...
			<div id="main_view" class="shadow">		
				...
				<div id="contenedor_contenido">		
			   		<?php if( isset($datos_artista) && ($datos_artista) != NULL){
			   			foreach ($datos_artista as $row) {?>
						...
						<nav id="perfil_artista_nav_header">
							<p><h2><?php echo $row['nombre_artista']; ?> </h2></p>							
							<?php if($this->session->userdata('logged_in') && $session_data['id_perfil']==1){
								//Si el usuario es administrador, permitir que edite el album si lo desea. ?>
								<span class="admin_edit"> <a href="<?php echo site_url('admin_edit/updateArtista/' . $row['id_artista'] ); ?>"> Editar artista </a> </span>
								<br />
								<span class="admin_edit"> <a href="<?php echo site_url('admin_edit/deleteArtista/' . $row['id_artista'] ); ?>"> Eliminar artista </a> </span>
								<br />
								<span class="admin_edit"> <a href="<?php echo site_url('admin_edit/insertAlbum/' . $row['id_artista'] ); ?>"> Agregar album </a> </span>
							...
							<h5>Genero: <?php echo  $row['genero_artista'] ?> </h5>
							<?php if($this->session->userdata('logged_in') && $session_data['id_perfil']==2){?>
								<form id="agregar_a_favoritos" method="post" action="<?=site_url('gestor_perfiles/agregarArtistaAFavoritos' )?>">
									
									<input name="id_artista" id="id_artista" type="hidden" style="DISPLAY: none;" class=""  value="<?php echo $row['id_artista'] ?>"/>
									<input name="id_usuario" id="id_usuario" type="hidden" style="DISPLAY: none;" class=""  value="<?php echo $session_data['id_usuario'] ?>"/>
									
									<input class="detalle_album_boton_descarga" type="submit" onclick="" value="Agregar a favoritos"/>
								</form>
							<?php } ?>
					   ...
					   <div class="contenedor_panel_busqueda">	
							...
							<div class="panel">
								<div class="panel_header">
									...
									<h2>
										<span>
											Albums de <?php echo $row['nombre_artista']; ?>
										</span>
									</h2>
								...
								<div class="body_panel">
									<?php 
										if(isset($albums_artista) && count($albums_artista) > 0){ 
											if (count($albums_artista) > 2){
									?>
										...
										?>
											<table class="tabla_resultados_busq">
												...
												<?php foreach($albums_artista as $row2) { ?>
													<tr>
														<td class="col1 center">
															<a href="<?php echo site_url('browser/ver_album/' . $row2['id_album'] ) ?>" class="img_busqueda"> <!-- TODO: Setear imagen del artista/album -->
																<img src="<?php echo site_url('imagenes/viewAlbum/' . $row2['id_album'] ); ?>" alt width="50">
															</a>
														</td>
														<td class="col2 center">
															<p class="perfil_artista_parrafo_info_general"><b>
																<a href="<?php echo site_url('browser/ver_album/' . $row2['id_album'] ) ?>" class="gray no_underline">
																	<?php echo  $row2['nombre_album'] ?>
																</a>
															</b></p>
															<p class="perfil_artista_parrafo_info_general"><?php echo  $row2['genero_album'] ?></p>
															<p class="perfil_artista_parrafo_info_general"><?php echo 'Cantidad downloads: <b>' .  $row2['cantidad_downloads'] . '</b>'; ?></p>
														</td>
														<td class="col2 center">
															 <?php echo  $row2['fecha_publicacion'] ?>
														</td>
														
														<td class="col2 center">
															<?php echo  floor($row2['rating']);  ?>
														</td>
														<td class="col2 center">
															<?php echo  $row2['cantidad_votos']  ?>
														</td>
														...
														?>
															...
															<a class="detalle_album_boton_descarga" href="<?php echo site_url("downloader/descargarAlbum/" .  $row2['id_album'] . "/" . $row2['nombre_album']);?>">Descargar</a>
									...
									<?php } else { ?>
										<div class="contenedor_tabla_resultados_busq_vacia">
											<table class="tabla_resultados_busq">
												<tr>										
													<td class="col2">  Aún no existen álbums asociados a este artista <em></em></td>
												</tr>	
											</table>
										</div>
									<?php } ?>
					...
					<?php } else { ?>
						<div class="center">
				   			<h1> El artista indicado no existe </h1>
				   		</div>
					<?php } ?>			
				</div> 
